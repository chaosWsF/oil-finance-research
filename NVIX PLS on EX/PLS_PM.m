...
parfor j=1:size(r, 2)
    ...
    % % rolling
    % for p=1:P
    %     X = [ones(R, 1) latent(p:p+R-1)];
    %     y = r_j(p:p+R-1);
    %     FC_val(p) = [1 latent(p+R)] * regress(y, X);
    % end
    % recursive
    for p=1:P
        X = [ones(R+p-1, 1) latent(1:R+p-1)];
        y = r_j(1:R+p-1);
        FC_val(p) = [1 latent(R+p)] * regress(y, X);
    end
...
writetable(tb1(oil_ex, :), "EX_results.xlsx", 'Sheet', "In_Sample_PLS", ...
    'WriteRowNames', true, 'WriteVariableNames', false, 'Range', 'A3');
writetable(tb1(oil_im, :), "EX_results.xlsx", 'Sheet', "In_Sample_PLS", ...
    'WriteRowNames', true, 'WriteVariableNames', false, 'Range', 'A13');
writetable(tb2(oil_ex, :), "EX_results.xlsx", 'Sheet', "Out_of_Sample_PLS", ...
    'WriteRowNames', true, 'WriteVariableNames', false, 'Range', 'A3');
writetable(tb2(oil_im, :), "EX_results.xlsx", 'Sheet', "Out_of_Sample_PLS", ...
    'WriteRowNames', true, 'WriteVariableNames', false, 'Range', 'A13');
