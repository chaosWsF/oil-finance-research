...
R = 6 * 12;
P = T - R;    % OOS period
results = NaN(size(r, 2), 3, N);    % beta, pVal, rsqr(%)
results_oos = NaN(size(r, 2), 2, N);    % rsqr_oos(%), cwstat
...
results = reshape(results, size(r, 2), 3 * N);
results_oos = reshape(results_oos, size(r, 2), 2 * N);
tb1 = array2table(results, 'RowNames', country_names);
tb2 = array2table(results_oos, 'RowNames', country_names);

oil_ex = [5 10 14 15 18 20 24];
oil_im = setdiff(1:24, oil_ex);
writetable(tb1(oil_ex, :), "EX_results.xlsx", 'Sheet', "In_Sample_DI", ...
    'WriteRowNames', true, 'WriteVariableNames', false, 'Range', 'A4');
writetable(tb1(oil_im, :), "EX_results.xlsx", 'Sheet', "In_Sample_DI", ...
    'WriteRowNames', true, 'WriteVariableNames', false, 'Range', 'A14');
writetable(tb2(oil_ex, :), "EX_results.xlsx", 'Sheet', "Out_of_Sample_DI", ...
    'WriteRowNames', true, 'WriteVariableNames', false, 'Range', 'A4');
writetable(tb2(oil_im, :), "EX_results.xlsx", 'Sheet', "Out_of_Sample_DI", ...
    'WriteRowNames', true, 'WriteVariableNames', false, 'Range', 'A14');
