...
Sharpe=nan(num_x*num_y,length(h));
...
for k=1:num_y
    ...
    for i=1:num_x
        ...
        Sharpe(row_num,1)=Sharpe_val_i_k-Sharpe_PM_k;
        AR(row_num,1)=(12/h)*(mean_without_nan(R_val_i_k))*0.97;
    end
end

xlswrite(output_file,CER_gain,'CER & Sharpe','c22');
xlswrite(output_file,Sharpe,'CER & Sharpe','d22');
xlswrite(output_file,AR,'CER & Sharpe','e22');

% xlswrite(output_file,CER_gain,'CER & Sharpe','h22');
% xlswrite(output_file,Sharpe,'CER & Sharpe','i22');
% xlswrite(output_file,AR,'CER & Sharpe','j22');