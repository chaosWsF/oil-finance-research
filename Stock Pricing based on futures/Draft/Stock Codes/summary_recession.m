...
nlag = 36;
mkdir('../../Pictures/paper1/industry/' + string(in_sample_period / 12));

% % green
% L = 3;
% in_sample_period = 1 * 12;
% nlag = 30;
% mkdir('./figures/green/' + string(in_sample_period / 12));
...
R2OS = nan(num_x*num_y, 2*size(recessions,1), length(h));
row_num = 0;
for k=1:num_y
    ...
    for i=1:num_x
        ...
        R2OS(row_num,:,1) = calr2os(u_mean_k, u_val_i_k, f_CW_i_k, ...
            nlag, start_date, in_sample_period, recessions);
    end
end

xlswrite('../../results/summary_ind_arabian.xlsx',R2OS,'Recession','b3');

function r2os=calr2os(u_mean,u_val,f_CW,nlag,start_date,in_sample,recessions)
    r2os = nan(2*size(recessions,1),1);
    ...
    for r=1:size(recessions,1)
        ...
        try
            ...
            % f_CW_r = f_CW(recession_start:recession_end);

            MSFE_val = sum(u_val_r.^2);
            MSFE_mean = sum(u_mean_r.^2);
            R2OS_val = 1 - MSFE_val/MSFE_mean;
            r2os(2*r,1) = R2OS_val';

            % results_CW = nwest(f_CW_r, ones(length(f_CW_r),1), nlag);
            % r2os(2*r-1,1) = 1 - normcdf(results_CW.tstat);
        catch
...
function plot_MSFE(start_date, in_sample, u_mean, u_val, period, k, recessions)
    ...
    figname = sprintf('../../Pictures/paper1/industry/%1d/industry%2d.png', in_sample / 12, k);
    % figname = sprintf('../../Pictures/paper1/green/%1d/green%2d.png', in_sample / 12, k);
    disp(figname)
    saveas(gcf, figname);
end